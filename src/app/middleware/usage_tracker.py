...
class ApiUsageTracker:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        if request_metadata["service_type"] in ['ask-agent', 'lookup-service', 'semantic-search']:
            # We'll need to get the usage_log_id first, so this is handled in the logger
            pass
        
        # Log usage
        await self._log_usage(
            request, response, user_info, request_metadata, 
            query_body_data, response_data, duration_ms
        )
        
        return response
    
    async def _log_usage(
        self, request, response, user_info, request_metadata, 
        query_body_data, response_data, duration_ms
    ):
        ...
        try:
            ...
            if (usage_log_id and 
                request_metadata["service_type"] in ['ask-agent', 'lookup-service', 'semantic-search']):
