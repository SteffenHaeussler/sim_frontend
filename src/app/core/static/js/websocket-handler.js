...
export class WebSocketHandler {
  ...
  _attemptConnection(resolve, reject) {
    console.log(
      `WebSocket connecting to: ${this.url} (attempt ${this.retryCount + 1})`,
    );
      ...
      this.websocket.onclose = (event) => {
        ...
        if (!this.isClosing && this.retryCount < this.options.maxRetries) {
          ...
          if (!this.isClosing) {
            reject(
              new Error(
                `WebSocket connection failed after ${this.retryCount} attempts`,
              ),
            );
          } else {
  ...
  _scheduleRetry(resolve, reject) {
    ...
    const delay = Math.min(
      this.options.baseDelay *
        Math.pow(this.options.backoffMultiplier, this.retryCount - 1),
      this.options.maxDelay,
    );

    console.log(
      `Retrying WebSocket connection in ${delay}ms (attempt ${this.retryCount}/${this.options.maxRetries})`,
    );

    if (this.options.onStatusUpdate) {
      this.options.onStatusUpdate(
        `Reconnecting... (${this.retryCount}/${this.options.maxRetries})`,
      );
    }
