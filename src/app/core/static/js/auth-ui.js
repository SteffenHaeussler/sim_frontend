...
class AuthUI {
  ...
  _initializeElements() {
    ...
    this.modals = [
      this.elements.loginModal,
      this.elements.registerModal,
      this.elements.forgotPasswordModal,
    ];
  }
  ...
  _setupEventListeners() {
    const listeners = {
      ...
      closeRegisterModal: () =>
        this._toggleModal(this.elements.registerModal, false),
      closeForgotPasswordModal: () =>
        this._toggleModal(this.elements.forgotPasswordModal, false),
      ...
      toggleLoginPassword: () =>
        this._togglePasswordVisibility(
          ...
          this.elements.toggleLoginPassword,
        ),
      toggleRegisterPassword: () =>
        this._togglePasswordVisibility(
          this.elements.registerPassword,
          this.elements.toggleRegisterPassword,
        ),
      toggleRegisterPasswordConfirm: () =>
        this._togglePasswordVisibility(
          this.elements.registerPasswordConfirm,
          this.elements.toggleRegisterPasswordConfirm,
        ),
    };
    ...
    document.addEventListener(
      "keydown",
      (e) => e.key === "Escape" && this.hideAllModals(),
    );
  }
  ...
  _handleForgotPasswordLink() {
    ...
    if (loginEmail.value && forgotEmail) {
      ...
      setTimeout(
        () => this._validateForm(this.elements.forgotPasswordForm),
        10,
      );
    }
    this._switchModal(
  ...
  _setupFormValidationListeners() {
    ...
    const formsToValidate = [
      { form: this.elements.loginForm, validator: this._isLoginFormValid },
      {
        ...
        validator: this._isRegisterFormValid,
      },
      {
        form: this.elements.forgotPasswordForm,
        validator: this._isForgotFormValid,
      },
    ];
    ...
    formsToValidate.forEach(({ form, validator }) => {
      if (form) {
        form.addEventListener("input", () =>
          this._validateForm(form, validator),
        );
    ...
    if (this.elements.registerPasswordConfirm) {
      this.elements.registerPasswordConfirm.addEventListener("input", () =>
  ...
  _validatePasswordMatch() {
    const { registerPassword, registerPasswordConfirm, passwordMatchFeedback } =
      this.elements;
    ...
    passwordMatchFeedback.textContent = isMatch
      ? "✓ Passwords match"
  ...
  async _handleApiCall(options) {
    const { button, errorDiv, apiCall, onSuccess, loadingText, buttonText } =
      options;
  ...
  handleLogin(e) {
    e.preventDefault();
    const { loginEmail, loginPassword, rememberMe, loginSubmit, loginError } =
      this.elements;
    this._handleApiCall({
      button: loginSubmit,
      errorDiv: loginError,
  ...
  updatePasswordStrength() {
    ...
    const strengthFill =
      this.elements.passwordStrengthIndicator.querySelector(".strength-fill");
    const strengthText =
      this.elements.passwordStrengthIndicator.querySelector(".strength-text");
  ...
  updateAuthState(isLoggedIn) {
    ...
    if (window.profileManager)
      window.profileManager.updateVisibility(isLoggedIn);
