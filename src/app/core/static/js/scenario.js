...
class ScenarioAgent {
  constructor() {
    ...
    this.sqlAgentHandler = new SqlAgentHandler(
    ...
    this.wsHandler = new WebSocketHandler({
      ...
      onMessage: (data, type) => {
        if (type === "json") {
          ...
          if (
            this.evaluationHandler.isInEvaluationMode() &&
            this.evaluationHandler.getSQLContainer()
          ) {
        ...
        if (status === "end") {
          ...
          if (messageBuffer.trim()) {
            if (
              ...
              if (!this.evaluationHandler.hasCurrentEvaluationDiv()) {
                this.evaluationHandler.addEvaluationToSQLContainer(
                  messageBuffer.trim(),
                );
              } else {
  ...
  handleWebSocketMessage(data, type) {
    ...
    if (type === "json") {
      console.log("First item check:", {
        ...
        hasEndpoint:
          data[0]?.endpoint !== undefined || data[0]?.end_point !== undefined,
      });
