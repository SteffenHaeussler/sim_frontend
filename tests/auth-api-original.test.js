...
describe("AuthAPI (Original)", () => {
  ...
  describe("token storage", () => {
    it("should calculate correct expiration timestamps", async () => {
      ...
      expect(localStorage.getItem("auth_token_expiry")).toBe(
        String(now + 3600 * 1000),
      );
      expect(localStorage.getItem("refresh_token_expiry")).toBe(
        String(now + 86400 * 1000),
      );
  ...
  describe("token refresh", () => {
    ...
    it("should throw error if no refresh token available", async () => {
      ...
      await expect(authAPI.refreshAccessToken()).rejects.toThrow(
