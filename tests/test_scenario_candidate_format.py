import pytest
from src.app.core.scenario_schema import ScenarioCandidate, ScenarioRecommendation
...
class TestScenarioCandidateFormat:
    ...
    def test_scenario_candidate_structure(self):
        ...
        candidate = ScenarioCandidate(
            sub_id="sub-1",
            question="SELECT temperature FROM sensors WHERE asset='DC-101'",
            endpoint="sqlagent"
        )
    ...
    def test_recommendation_with_candidates(self):
        ...
        candidates = [
            ScenarioCandidate(
                sub_id="sub-1",
                question="SELECT temperature, pressure FROM dc101_params",
                endpoint="sqlagent"
            ),
            ScenarioCandidate(
                sub_id="sub-2",
                question="Get real-time sensor data for DC-101",
                endpoint="toolagent"
            )
        ]
