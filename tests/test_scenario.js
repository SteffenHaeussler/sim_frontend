...
vi.mock("../src/app/core/static/js/sql-agent-handler.js", () => ({
  SqlAgentHandler: vi.fn().mockImplementation(() => ({
    ...
    generateSessionId: vi.fn(
      () => "sql-" + Date.now() + "-" + Math.random().toString(36).substr(2, 9),
    ),
    createRequestCard: vi.fn((request) => {
      ...
      card.innerHTML = `
                ...
                <div class="sql-card-question">Question: ${
                  request.question
                }</div>
...
describe("ScenarioAgent", () => {
  ...
  describe("Initialization", () => {
    it("should initialize with correct elements", () => {
      expect(scenarioAgent.messagesElement).toBe(
        document.getElementById("messages"),
      );
      expect(scenarioAgent.questionInput).toBe(
        document.getElementById("question"),
      );
      expect(scenarioAgent.sendButton).toBe(
  ...
  describe("WebSocket Message Handling", () => {
    it.skip("should handle SQL agent request JSON data", () => {
      ...
      const handleSQLSpy = vi.spyOn(
        scenarioAgent.sqlAgentHandler,
        "handleRequests",
      );
    ...
    it.skip("should handle evaluation mode messages", () => {
      ...
      scenarioAgent.evaluationHandler.getSQLContainer.mockReturnValue(
        document.createElement("div"),
      );

      const addEvalSpy = vi.spyOn(
        scenarioAgent.evaluationHandler,
        "addEvaluationToSQLContainer",
      );
    ...
    it.skip("should handle end event", () => {
      ...
      expect(
        scenarioAgent.evaluationHandler.setEvaluationMode,
  ...
  describe("SQL Request Card Creation", () => {
    it("should create SQL request card with correct structure", () => {
      ...
      expect(card.querySelector(".sql-endpoint").textContent).toBe(
        "/sql-endpoint",
      );
      expect(card.querySelector(".sql-card-question").textContent).toContain(
        "Test SQL question",
      );
    });
  ...
  describe("Action Buttons", () => {
    it("should copy content to clipboard", async () => {
      ...
      await scenarioAgent.sqlAgentHandler.copyToClipboard(
        "Test content",
        copyBtn,
      );
    ...
    it("should handle rating submission", async () => {
      ...
      await scenarioAgent.sqlAgentHandler.rateResponse(
        request,
        card,
        "up",
        upBtn,
  ...
  describe("HTML Sanitization", () => {
    it("should sanitize message content", async () => {
      ...
      scenarioAgent.messageRenderer.addMessage.mockImplementation(
        (content, isQuestion) => {
          if (!isQuestion) {
      ...
      scenarioAgent.messageRenderer.addMessage(
    ...
    it("should escape HTML in createSQLRequestCard", async () => {
      ...
      scenarioAgent.sqlAgentHandler.createRequestCard.mockImplementation(
        (request) => {
      ...
      expect(htmlSanitizer.escapeHtml).toHaveBeenCalledWith(
