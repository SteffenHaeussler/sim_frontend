...
class TestSessionManagement:
    ...
    def test_agent_endpoint_accepts_custom_session_id(self, client, auth_headers, mock_httpx_client):
        ...
        response = client.get(f"/agent?question=test question&session_id={custom_session_id}", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_asset_endpoints_support_session_id(self, client, auth_headers, mock_httpx_client):
        ...
        response = client.get(f"/api/asset/asset_001?session_id={custom_session_id}", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_lookup_endpoints_support_session_id(self, client, auth_headers):
        ...
        response = client.get(f"/lookup/assets?session_id={custom_session_id}", headers=auth_headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
