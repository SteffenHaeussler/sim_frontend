...
describe("AssetSearch", () => {
  ...
  beforeEach(() => {
    ...
    window.app = {
      getTrackingHeaders: vi
        .fn()
        .mockReturnValue({ "X-Session-ID": "test-123" }),
    };

    assetSearch = new window.AssetSearch();

    // Initialize the DOM element references manually since DOMContentLoaded won't fire
    assetSearch.assetNameSearch =
      window.document.getElementById("asset-name-search");
    assetSearch.assetTypeFilter =
      window.document.getElementById("asset-type-filter");
    assetSearch.typeFilter = window.document.getElementById("type-filter");
    assetSearch.clearFiltersBtn =
      window.document.getElementById("clear-filters");
  ...
  describe("searchAssets", () => {
    beforeEach(() => {
      ...
      assetSearch.typeFilter = window.document.getElementById("type-filter");
      assetSearch.assetList = window.document.getElementById("asset-list");
      assetSearch.resultsCount =
        window.document.getElementById("results-count");
    });
  ...
  describe("pagination", () => {
    ...
    it("should change page when clicking navigation", async () => {
      ...
      const searchSpy = vi
        .spyOn(assetSearch, "searchAssets")
        .mockImplementation(() => {});
