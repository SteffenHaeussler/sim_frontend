...
describe('AskAgent', () => {
  ...
  describe('initialization', () => {
    ...
    it('should set up event listeners', () => {
      const handleSendSpy = vi.spyOn(askAgent, 'handleSendMessage').mockImplementation(() => {});
  ...
  describe('addMessage', () => {
    ...
    it('should add AI response with markdown', () => {
      ...
      const messages = askAgent.messagesElement.querySelectorAll('.message');
      expect(messages.length).toBe(1);
      expect(global.marked.parse).toHaveBeenCalledWith('The **total revenue** is $1M');
      expect(messages[0].innerHTML).toContain('<p>The **total revenue** is $1M</p>');
    });
  ...
  describe('copyMessage', () => {
    ...
    it('should use fallback method when clipboard API not available', async () => {
      ...
      const fallbackSpy = vi.spyOn(askAgent, 'copyToClipboardFallback').mockImplementation(() => {});
      const button = document.createElement('button');
