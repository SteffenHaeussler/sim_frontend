...
class TestScenarioValidation:
    ...
    def test_script_tag_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScenarioQueryMessage(
                type="query",
                query="What is <script>alert('XSS')</script> the temperature?",
                message_id="scenario-123"
            )
    ...
    def test_javascript_url_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScenarioQueryMessage(
                type="query",
                query="Check this javascript:alert('XSS')",
                message_id="scenario-123"
            )
    ...
    def test_event_handler_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScenarioQueryMessage(
                type="query",
                query="What is the onerror=alert('XSS') value?",
                message_id="scenario-123"
            )
    ...
    def test_invalid_message_id_format(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ScenarioQueryMessage(
                type="query",
                query="Valid query",
                message_id="scenario@123#invalid"
            )
