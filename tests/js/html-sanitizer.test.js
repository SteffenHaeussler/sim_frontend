...
describe("HTMLSanitizer", () => {
  ...
  it("should remove data: URLs except images", () => {
    const html =
      "<a href=\"data:text/html,<script>alert('XSS')</script>\">Bad</a>";
    const result = sanitizer.sanitize(html);
  ...
  it("should preserve allowed attributes", () => {
    const html =
      '<div class="container" id="main"><p class="text">Hello</p></div>';
    const result = sanitizer.sanitize(html);
  ...
  it("should handle nested malicious content", () => {
    const html =
      '<div><p>Safe</p><script><script>alert("XSS")</script></script></div>';
    const result = sanitizer.sanitize(html);
    expect(result).not.toContain("<script>");
