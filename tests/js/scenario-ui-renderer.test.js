...
describe("ScenarioUIRenderer", () => {
  ...
  it("should update recommendations correctly", () => {
    ...
    const recommendations = [
      {
        sub_id: "sub-1",
        question: "Get tank level data from SQL",
        endpoint: "sqlagent",
      },
      {
        sub_id: "sub-2",
        question: "Analyze tank level trends",
        endpoint: "toolagent",
      },
      {
        sub_id: "sub-3",
        question: "Check for anomalies",
        endpoint: "sqlagent",
      },
    ];
  ...
  it("should handle partial results correctly", () => {
    ...
    let statusSpan = container.querySelector(
      `[data-sub-id="${subId}"] .result-status`,
    );
    expect(statusSpan.textContent).toBe("⏳");
    ...
    statusSpan = container.querySelector(
      `[data-sub-id="${subId}"] .result-status`,
    );
    expect(statusSpan.textContent).toBe("✓");
  });
  ...
  it("should update completion status correctly", () => {
    ...
    // Partial completion
    renderer.updateStatus(messageId, {
      completed: 2,
      total: 5,
      percentage: 40,
    });
  ...
  it("should handle missing container gracefully", () => {
    ...
    expect(() => {
      renderer.updateRecommendations("non-existent", [
        { sub_id: "sub-1", question: "test", endpoint: "sqlagent" },
      ]);
    }).not.toThrow();
  ...
  it("should prevent XSS in recommendations", () => {
    ...
    const maliciousRecs = [
      {
        sub_id: "sub-1",
        question: '<script>alert("XSS")</script>Safe text',
        endpoint: "sqlagent",
      },
      {
        sub_id: "sub-2",
        question: "Normal recommendation",
        endpoint: "toolagent",
      },
      {
        sub_id: "sub-3",
        question: "<img src=x onerror=\"alert('XSS')\">",
        endpoint: "<script>bad</script>",
      },
    ];
  ...
  it("should sanitize HTML in result content", () => {
    ...
    const maliciousContent =
      '<p>Safe content</p><script>alert("XSS")</script><b>Bold text</b>';
